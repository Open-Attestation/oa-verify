...
describe("OpenAttestationEthereumDocumentStoreStatus", () => {
  ...
  describe("test", () => {
    it("should return true when v2 document has at least one certificate store", () => {
      const test = openAttestationEthereumDocumentStoreStatus.test(documentRopstenNotIssuedWithCertificateStore, {
        network: "ropsten",
      });
      expect(test).toStrictEqual(true);
    });
    it("should return true when v2 document has at least one document store", () => {
      const test = openAttestationEthereumDocumentStoreStatus.test(documentRopstenNotIssuedWithDocumentStore, {
        network: "ropsten",
      });
      expect(test).toStrictEqual(true);
    });
    it("should return false when v2 document uses token registry", () => {
      const test = openAttestationEthereumDocumentStoreStatus.test(documentRopstenNotIssuedWithTokenRegistry, {
        network: "ropsten",
      });
      expect(test).toStrictEqual(false);
    });
    it("should return true when v3 document uses DOCUMENT_STORE method", () => {
      const test = openAttestationEthereumDocumentStoreStatus.test(v3documentRopstenValidWithDocumentStore, {
        network: "ropsten",
      });
      expect(test).toStrictEqual(true);
    });
    it("should return false when v3 document uses TOKEN_REGISTRY method", () => {
      const test = openAttestationEthereumDocumentStoreStatus.test(documentRopstenValidWithTokenRegistry, {
        network: "ropsten",
      });
      expect(test).toStrictEqual(false);
    });
  ...
  describe("v3", () => {
    it("should return an invalid fragment when document with document store that has been revoked", async () => {
      const fragment = await openAttestationEthereumDocumentStoreStatus.verify(documentRopstenRevoked, {
        network: "ropsten",
      });
      expect(fragment).toStrictEqual({
        name: "OpenAttestationEthereumDocumentStoreStatus",
